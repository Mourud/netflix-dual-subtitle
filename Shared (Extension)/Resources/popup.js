...
// let API_KEY = await browser.storage.local.get("api_key")?.api_key;
...
function switchContainer(container, display) {
  Object.values(containers).forEach((c) => {
    c.style.display = "none";
    // disable all buttons and events
  });
  containers[container].style.display = display;
  // enable all buttons and events
}
...
async function isAPIKeyValid(API_KEY){
  ...
  try {
    ...
    if (!response.ok) {
      // Check if response is not successful (e.g., 401 Unauthorized)
...
const containers = {
  api : document.getElementById("apiContainer"),
...
if(API_KEY){
  ...
  switchContainer("api", "flex");
}

const apiButton = document.getElementById("submitButton");
apiButton.addEventListener("click", async () => {
  ...
  if (await isAPIKeyValid(key)) {
    ...
    // apiButton.style.backgroundColor = "red";
    apiButton.textContent = "Try Again";
  }
});